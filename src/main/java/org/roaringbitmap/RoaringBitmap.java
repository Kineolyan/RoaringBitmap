...
public class RoaringBitmap implements Cloneable, Serializable, Iterable<Integer>, Externalizable,
	...
	protected static StorageArray and(final StorageArray a1, final StorageArray a2) {
		if (a1 instanceof RoaringArray && a2 instanceof RoaringArray) {
			return a1.and(a2);
		} else if (a1 instanceof MewingArray) {
			return a1.and(a2);
		} else if (a2 instanceof MewingArray) {
			return a2.and(a1);
		} else {
			throw new IllegalStateException("Unknown type for args " + a1 + " and " + a2);
		}
	}
	 ...
	 * @param b2 other bitmap
	 * @return as if you did and(x2,x2).getCardinality()
	 * @see FastAggregation#and(RoaringBitmap...)
	 */
	protected static int andCardinality(final RoaringBitmap b1, final RoaringBitmap b2) {
		if (b1.highLowContainer instanceof RoaringArray && b2.highLowContainer instanceof RoaringArray) {
			return b1.highLowContainer.andCardinality(b2.highLowContainer);
		} else if (b1.highLowContainer instanceof MewingArray) {
			return b1.highLowContainer.andCardinality(b2.highLowContainer);
		} else if (b2.highLowContainer instanceof MewingArray) {
			return b2.highLowContainer.andCardinality(b1.highLowContainer);
		} else {
			throw new IllegalStateException("Unknown type for args " + b1 + " and " + b2);
		}
	}
	 ...
	 * @return result of the operation
	 */
	public static RoaringBitmap andNot(final RoaringBitmap b1, final RoaringBitmap b2) {
		return new RoaringBitmap(andNot(b1.highLowContainer, b2.highLowContainer));
	}

	/**
	 * Bitwise ANDNOT (difference) operation. The provided arrays are *not* modified. This operation
	 * is thread-safe as long as the provided bitmaps remain unchanged.
	 * @param a1 first storage array
	 * @param a2 other storage array
	 * @return result of the operation
	 */
	public static StorageArray andNot(final StorageArray a1, final StorageArray a2) {
		if (a1 instanceof RoaringArray && a2 instanceof RoaringArray) {
			return a1.andNot(a2);
		} else if (a1 instanceof MewingArray) {
			return a1.andNot(a2);
	 ...
	 */
	public static boolean intersects(final RoaringBitmap b1, final RoaringBitmap b2) {
		if (b1.highLowContainer instanceof RoaringArray && b2.highLowContainer instanceof RoaringArray) {
			return b1.highLowContainer.intersects(b2.highLowContainer);
		} else if (b1.highLowContainer instanceof MewingArray) {
			return b1.highLowContainer.intersects(b2.highLowContainer);
		} else if (b2.highLowContainer instanceof MewingArray) {
			return b2.highLowContainer.intersects(b1.highLowContainer);
		} else {
			throw new IllegalStateException("Unknown type for args " + b1 + " and " + b2);
		}
	}
	...
	public static RoaringBitmap or(final RoaringBitmap x1, final RoaringBitmap x2) {
		return new RoaringBitmap(or(x1.highLowContainer, x2.highLowContainer));
	}

	protected static StorageArray or(final StorageArray a1, final StorageArray a2) {
		if (a1 instanceof MewingArray && a2 instanceof MewingArray) {
			return a1.or(a2);
		} else if (a1 instanceof RoaringArray) {
			return a1.or(a2);
		} else if (a2 instanceof RoaringArray) {
			return a2.or(a1);
		} else {
			throw new IllegalStateException("Unknown type for args " + a1 + " and " + a2);
		}
	...
	public static RoaringBitmap remove(RoaringBitmap rb, final long rangeStart, final long rangeEnd) {
		...
		final StorageArray container = rb.highLowContainer.remove(rangeStart, rangeEnd);
		return new RoaringBitmap(container);
	}
	...
	public static RoaringBitmap xor(final RoaringBitmap x1, final RoaringBitmap x2) {
		// TODO may try to find a good heuristic here
		return new RoaringBitmap(x1.highLowContainer.xor(x2.highLowContainer));
	}
	...
	public RoaringBitmap(ImmutableRoaringBitmap rb) {
		if (rb.getCardinality() < MewingArray.DEFAULT_MAX_SIZE) {
			final MewingArray array = new MewingArray();
			// FIXME do something clever as we know the container is correct. Or use forEach
			// But we know that each value is not in the array
	...
	public void and(final RoaringBitmap x2) {
		final StorageArray a1 = this.highLowContainer;
		final StorageArray a2 = x2.highLowContainer;
		if (a1 instanceof RoaringArray && a2 instanceof RoaringArray) {
			this.highLowContainer = a1.iand(a2);
		} else if (a1 instanceof MewingArray) {
			this.highLowContainer =  a1.iand(a2);
		} else if (a2 instanceof MewingArray) {
			this.highLowContainer =  a2.iand(a1);
		} else {
			throw new IllegalStateException("Unknown type for args " + a1 + " and " + a2);
		}
	}
   ...
   */
	...
	public void andNot(final RoaringBitmap x2) {
		...
		final StorageArray a2 = x2.highLowContainer;
		if (a1 instanceof RoaringArray && a2 instanceof RoaringArray) {
			this.highLowContainer = a1.iandNot(a2);
		} else if (a1 instanceof MewingArray) {
			this.highLowContainer =  a1.iandNot(a2);
		} else if (a2 instanceof MewingArray) {
			this.highLowContainer =  a2.iandNot(a1);
		} else {
			throw new IllegalStateException("Unknown type for args " + a1 + " and " + a2);
		}
	}
...
//    return ((RoaringArray) this.highLowContainer).getContainerPointer();
	...
	public PeekableIntIterator getIntIterator() {
		return this.highLowContainer.getIntIterator();
	}
	...
	public IntIterator getReverseIntIterator() {
		return this.highLowContainer.getReverseIntIterator();
	}

	@Override
	public Iterator<Integer> iterator() {
		return this.highLowContainer.iterator();
	}
	...
	public void or(final RoaringBitmap x2) {
		...
		final StorageArray a2 = x2.highLowContainer;
		if (a1 instanceof MewingArray && a2 instanceof MewingArray) {
			this.highLowContainer = a1.ior(a2);
		} else if (a1 instanceof RoaringArray) {
			this.highLowContainer =  a1.ior(a2);
		} else if (a2 instanceof RoaringArray) {
			this.highLowContainer =  a2.ior(a1);
		} else {
			throw new IllegalStateException("Unknown type for args " + a1 + " and " + a2);
		}
	}
	...
	public void xor(final RoaringBitmap x2) {
		// FIXME find a good heuristic for this op
		this.highLowContainer = this.highLowContainer.ixor(x2.highLowContainer);
	}
